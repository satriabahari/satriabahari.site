...
const ItemHovered = ({ data, type }: ItemHoveredProps) => (
  <div className="flex flex-col items-center gap-y-1 text-xs">
    ...
    <span className="dark:text-neutral-50">{`${Math.floor(data?.maxWpm.raw)} raw`}</span>
    <span className="dark:text-neutral-50">{`${Math.floor(data?.maxWpm.acc)}% acc`}</span>
    <span className="dark:text-neutral-50">{`${Math.floor(data?.maxWpm.consistency)}% con`}</span>
